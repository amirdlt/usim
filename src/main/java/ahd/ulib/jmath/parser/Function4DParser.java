...
import ahd.ulib.jmath.datatypes.functions.Function4D;
import ahd.ulib.jmath.datatypes.functions.TernaryFunction;
import ahd.ulib.jmath.functions.utils.FunctionUtil;
import ahd.ulib.jmath.functions.utils.InverseFinder;
import ahd.ulib.jmath.operators.*;
...
public class Function4DParser implements Parser<Function4D> {
    ...
    private Function4D doOrderOfOperations(TokenString tokens) {
        ...
        if (location != -1) {
            ...
            res = FunctionUtil.sum(doOrderOfOperations(left), doOrderOfOperations(right));
        } else {
            location = scanFromRight(tokens, TokenType.MINUS);
            if (location != -1) {
                TokenString left = tokens.split(0, location);
                TokenString right = tokens.split(location + 1, tokens.getLength());
                res = FunctionUtil.sub(doOrderOfOperations(left), doOrderOfOperations(right));
            } else {
                location = scanFromRight(tokens, TokenType.DIVIDED_BY);
                if (location != -1) {
                    TokenString left = tokens.split(0, location);
                    TokenString right = tokens.split(location + 1, tokens.getLength());
                    res = FunctionUtil.fraction(doOrderOfOperations(left), doOrderOfOperations(right));
                } else {
                    location = scanFromRight(tokens, TokenType.TIMES);
                    if (location != -1) {
                        TokenString left = tokens.split(0, location);
                        TokenString right = tokens.split(location + 1, tokens.getLength());
                        res = FunctionUtil.multiply(doOrderOfOperations(left), doOrderOfOperations(right));
                    } else {
                        location = scanFromRight(tokens, TokenType.MODULO);
                        if (location != -1) {
                            TokenString left = tokens.split(0, location);
                            TokenString right = tokens.split(location + 1, tokens.getLength());
                            res = FunctionUtil.modulo(doOrderOfOperations(left), doOrderOfOperations(right));
                        } else {
                            location = scanFromRight(tokens, TokenType.RAISED_TO);
                            if (location != -1) {
                                TokenString left = tokens.split(0, location);
                                TokenString right = tokens.split(location + 1, tokens.getLength());
                                res = FunctionUtil.power(doOrderOfOperations(left), doOrderOfOperations(right));
                            } else {
